...
def control_loop(dummy, state):
    ...
    sensor = MAX31855.MAX31855(spi=SPI.SpiDev(conf.spi_port,
                               conf.spi_dev))
    ...
    try:
      ...
      # If steam button pressed, change setpoint temperature and observer gain
          ...
          # Change state instantaneously (by-pass filter)
                ...
                settemp = state['settemp']
                x = x - numpy.mat([[settemp], [settemp]]) \
                    + numpy.mat([[lastsettemp], [lastsettemp]])
                x_prev = x
                lastsettemp = state['settemp']
      ...
      # If brewing, add feed forward control and change observer
            ...
            if brew_state:  # Not brewing
                x = A * x_prev + B * u + B2 + K * conf.sample_time * (y
                        - y_tilde) * numpy.mat('1; 1')
                d_1vec = numpy.zeros((n, 1))
            else:

            # Brewing

                x = A * x_prev + B * (u - conf.brew_boost) + B2 + K \
                    * conf.sample_time * (y - y_tilde) \
                    * numpy.mat('1; 1')
      ...
      # Check if timer is on
          ...
          # Equality constraint
                ...
                if brew_state:
                    ...
                    b_constr = mpc_mat['A_app'] * x \
                        + mpc_mat['b_constr_brew']
          ...
          # Integral control, only if not steaming
            ...
            print 'Exec. time:', str(exec_time), 'Temperature:', \
                state['tempc'], 'Control signal:', \
                state['control_signal'], 'Integral control:', \
                round(u_I, 2), 'Awake:', str(awake), 'Temp. setpoint:', \
                str(settemp), 'y:', str(round(x.item(1, 0), 2))
...
Traceback (most recent call last):
  ...
  File "build/bdist.linux-x86_64/egg/PythonTidy.py", line 3498, in put
  File "build/bdist.linux-x86_64/egg/PythonTidy.py", line 4343, in put
  File "build/bdist.linux-x86_64/egg/PythonTidy.py", line 4462, in put
  File "build/bdist.linux-x86_64/egg/PythonTidy.py", line 4343, in put
  File "build/bdist.linux-x86_64/egg/PythonTidy.py", line 4709, in put
  File "build/bdist.linux-x86_64/egg/PythonTidy.py", line 4343, in put
  File "build/bdist.linux-x86_64/egg/PythonTidy.py", line 2310, in put
  File "build/bdist.linux-x86_64/egg/PythonTidy.py", line 2121, in put
  File "build/bdist.linux-x86_64/egg/PythonTidy.py", line 1760, in line_term
  File "build/bdist.linux-x86_64/egg/PythonTidy.py", line 2152, in get_hi_lineno
